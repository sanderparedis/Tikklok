...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex bg-brand-bg text-slate-800 font-sans h-screen overflow-hidden">
      {/* Left Sidebar Nav */}
      <aside className="w-20 bg-brand-sidebar flex flex-col items-center py-8 gap-10 shrink-0">
      ...
      <main className="flex-1 flex flex-col p-8 gap-6 overflow-y-auto">
        ...
        <header className="flex justify-between items-end shrink-0">
          <div>
            ...
            <p className="text-slate-500 font-medium">Overzicht van je professionele activiteiten</p>
          </div>
          <div className="flex gap-4">
            <div className="bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-200 min-w-32">
              <span className="label-tiny">Doelstelling</span>
              <span className="text-lg mono-value">30:35</span>
            </div>
            <div className="bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-200 min-w-32">
              <span className="label-tiny">Gewerkt</span>
              <span className={`text-lg mono-value ${progressPercent >= 100 ? 'text-green-600' : 'text-brand-primary'}`}>
        ...
        <div className="grid grid-cols-12 gap-6 flex-1 min-h-0">
          ...
          {/* Input Section (Left) */}
          <section className="col-span-4 flex flex-col gap-6 overflow-y-auto pr-2 pb-4">
            {activeTab === 'hours' && (
          ...
          <section className="col-span-8 flex flex-col gap-6 min-h-0">
            ...
            <div className="card-panel p-6 shrink-0">
                  ...
                  {activeTab === 'hours' ? (
                    ...
                    >
                      {groupedMonthlyData.map(([key, stats]) => (
                        <div key={key} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:border-brand-primary/30 transition-all group">
                          <div className="flex justify-between items-start mb-6">
                            <div>
                              <h4 className="text-lg font-bold text-slate-900 capitalize">{key}</h4>
                              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Maandrapport</p>
                            </div>
                            <button 
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                              ...
                              <span className="block text-xl mono-value text-slate-900">{formatMonoTime(stats.workMin)}</span>
                            </div>
                            <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                              <span className="label-tiny">Reiskosten</span>
                              <span className="block text-xl mono-value text-green-600">€{stats.travelComp.toFixed(2)}</span>
                            </div>
                            <div className="col-span-2 flex items-center justify-between px-2 pt-2 border-t border-slate-100">
                              <span className="text-[10px] font-bold text-slate-400 uppercase">Totaal kilometers</span>
